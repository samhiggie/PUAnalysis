...
void makeHistograms2D(TString inputPath, std::string channel_, TString luminosity);
std::vector<float> getFFractions(float m_sv);
int findBin(std::string histname,float m_sv);
std::vector<float> getFFractions2D(float m_vis_val, float njets_val);
std::pair<int,int> findBin2D(std::string histname,float m_vis,float njets);
...
int main (int argc, char* argv[]) 
{
    ...
    makeHistograms2D(inputPath,channel,"59740.0");//last number is lumi in pb
...
void readdir(TDirectory *dir, optutl::CommandLineParser parser,  char TreeToUse[]) 
{
    ...
    while ((key = (TKey*)next())) {
        ...
        if (obj->IsA()->InheritsFrom(TDirectory::Class())) {
            ...
            TString ff_file_name = "/afs/hep.wisc.edu/home/samuellh/WorkingArea/CPstuff/Iso_2017/src/HTTutilities/Jet2TauFakes/data/SM2018/tight/vloose/mt/fakeFactors.root";
            ...
            for(int i=0;i<t->GetEntries();i++ ){
                ...
                if( isoTight_2<0.5){
                    ...
                    fractions  = getFFractions2D((float)m_vis,(float)njets);
...
void makeHistograms(TString path,std::string channel_,TString luminosity){
    ...
    std::vector<float> m_sv{0,50,80,100,110,120,130,150,170,200,250,1000};
    ...
    TH1D *hW   = new TH1D("W","W",10,50.0,250.0);
    TH1D *hZJ   = new TH1D("ZJ","ZJ",10,50.0,250.0); 
    TH1D *hVVJ = new TH1D("VVJ","VVJ",10,50.0,250.0);
    TH1D *hVVT = new TH1D("VVT","VVT",10,50.0,250.0);
    TH1D *hTTJ   = new TH1D("TTJ","TTJ",10,50.0,250.0);
    TH1D *hTTT   = new TH1D("TTT","TTT",10,50.0,250.0);
    TH1D *hZTT   = new TH1D("ZTT","ZTT",10,50.0,250.0);
    TH1D *hdata    = new TH1D("data","data",10,50.0,250.0);
    ...
    TString preselection = "pt_1>21&&pt_2>30&&npv>0&&id_m_medium_1>0&&tightMuons<=1&&tightElectrons==0&&diLeptons==0&&againstMuonTight3_2>0&&againstElectronVLooseMVA6_2>0&&HLT_Any>0&&mt_1<50&&";
    ...
    //TString weight_ ="__WEIGHT__*GENWEIGHT*"+luminosity;
    ...
    twjets->Draw("m_sv>>W", "("+preselection+ApplicationRegion+fakeSelection+")*"+weight_)     ;// TH2D *hW = gDirectory->Get("hW");
    tzjets->Draw("m_sv>>ZJ", "("+preselection+ApplicationRegion+fakeSelection+")*"+weight_)    ;// TH2D *hZJ = gDirectory->Get("hZJ");
    tdiboson->Draw("m_sv>>VVJ", "("+preselection+ApplicationRegion+fakeSelection+")*"+weight_) ;// TH2D *hVVJ = gDirectory->Get("hVVJ");
    tdiboson->Draw("m_sv>>VVT", "("+preselection+ApplicationRegion+trueSelection+")*"+weight_) ;// TH2D *hVVT = gDirectory->Get("hVVT");
    tttbar->Draw("m_sv>>TTJ", "("+preselection+ApplicationRegion+fakeSelection+")*"+weight_)   ;// TH2D *hTTJ = gDirectory->Get("hTTJ");
    tttbar->Draw("m_sv>>TTT", "("+preselection+ApplicationRegion+trueSelection+")*"+weight_)   ;// TH2D *hTTT = gDirectory->Get("hTTT");
    //tttbar->Draw("m_sv>>ZTT", "("+preselection+ApplicationRegion+trueSelection+")*"+weight_)   ;// TH2D *hZTT = gDirectory->Get("hZTT");
    tzjets->Draw("m_sv>>ZTT", "("+preselection+ApplicationRegion+trueSelection+")*"+weight_)   ;// TH2D *hZTT = gDirectory->Get("hZTT");
    tdata->Draw("m_sv>>data",preselection+ApplicationRegiondata)        ;// TH2D *hdata = gDirectory->Get("hdata");
...
void makeHistograms2D(TString path,std::string channel_,TString luminosity){
    //using namespace ROOT;
    //TString path = "/hdfs/store/user/samuellh/HTT_2018_Oct23_corr/";
    //TString path = inputPath.c_str();
    //making the cut string
...
std::vector<float> getFFractions(float m_sv) {
    ...
    int thebin;
    thebin=findBin("W",m_sv);
    //std::cout<<"Found the bin!"<<std::endl;
    //std::cout<<"The bins!  "<<thebins.first<<"    "<<thebins.second<<std::endl;
    
     
    float W    = ((TH1D*) histfile->Get("W"   ))->GetBinContent(thebin); 
    float ZJ   = ((TH1D*) histfile->Get("ZJ"  ))->GetBinContent(thebin);
    float VVJ  = ((TH1D*) histfile->Get("VVJ" ))->GetBinContent(thebin);
    float VVT  = ((TH1D*) histfile->Get("VVT" ))->GetBinContent(thebin);
    float TTT  = ((TH1D*) histfile->Get("TTT" ))->GetBinContent(thebin);
    float TTJ  = ((TH1D*) histfile->Get("TTJ" ))->GetBinContent(thebin);
    float ZTT  = ((TH1D*) histfile->Get("ZTT" ))->GetBinContent(thebin);
    float data = ((TH1D*) histfile->Get("data"))->GetBinContent(thebin);
...
std::vector<float> getFFractions2D(float m_vis_val, float njets_val) {
    ...
    thebins=findBin2D("W",m_vis_val,njets_val);
    //std::cout<<"Found the bin!"<<std::endl;
    //std::cout<<"The bins!  "<<thebins.first<<"    "<<thebins.second<<std::endl;
    ...
    float ZTT  = ((TH2D*) histfile->Get("ZTT" ))->GetBinContent(thebins.first,thebins.second);
