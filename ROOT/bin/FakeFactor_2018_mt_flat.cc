...
void makeHistograms(TString path,std::string channel_,TString luminosity){
    ...
    twjets->Draw("m_vis:njets>>W", "("+preselection+ApplicationRegion+fakeSelection+")*"+weight_)     ;// TH2D *hW = gDirectory->Get("hW");
    tzjets->Draw("m_vis:njets>>ZJ", "("+preselection+ApplicationRegion+fakeSelection+")*"+weight_)    ;// TH2D *hZJ = gDirectory->Get("hZJ");
    tdiboson->Draw("m_vis:njets>>VVJ", "("+preselection+ApplicationRegion+fakeSelection+")*"+weight_) ;// TH2D *hVVJ = gDirectory->Get("hVVJ");
    tdiboson->Draw("m_vis:njets>>VVT", "("+preselection+ApplicationRegion+trueSelection+")*"+weight_) ;// TH2D *hVVT = gDirectory->Get("hVVT");
    tttbar->Draw("m_vis:njets>>TTJ", "("+preselection+ApplicationRegion+fakeSelection+")*"+weight_)   ;// TH2D *hTTJ = gDirectory->Get("hTTJ");
    tttbar->Draw("m_vis:njets>>TTT", "("+preselection+ApplicationRegion+trueSelection+")*"+weight_)   ;// TH2D *hTTT = gDirectory->Get("hTTT");
    //tttbar->Draw("m_vis:njets>>ZTT", "("+preselection+ApplicationRegion+trueSelection+")*"+weight_)   ;// TH2D *hZTT = gDirectory->Get("hZTT");
    tzjets->Draw("m_vis:njets>>ZTT", "("+preselection+ApplicationRegion+trueSelection+")*"+weight_)   ;// TH2D *hZTT = gDirectory->Get("hZTT");
    tdata->Draw("m_vis:njets>>data",preselection+ApplicationRegiondata)        ;// TH2D *hdata = gDirectory->Get("hdata");
