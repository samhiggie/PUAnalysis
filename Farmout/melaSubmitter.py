...
#SVFitStandAlone outputFile="WZ.root" newOutputFile=1.0 newFile="none"
    ...
    #####bashScript += '$CMSSW_BASE/bin/$SCRAM_ARCH/MELA_StandAlone newOutputFile=1 inputfile=$value newFile=\'$OUTPUT\'' #% (channel, sample_name, period)
    bashScript += '$CMSSW_BASE/bin/$SCRAM_ARCH/MELA_StandAlone_SM_AC newOutputFile=1 inputfile=$value newFile=\'$OUTPUT\'' #% (channel, sample_name, period)
...
# create farmout command
    farmoutString = 'farmoutAnalysisJobs --max-usercode-size=200 --site-requirements=\'OpSysAndVer == "SL6"\' --infer-cmssw-path --fwklite --input-file-list=%s' % (input_name)
