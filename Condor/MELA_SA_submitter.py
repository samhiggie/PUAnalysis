...
def main(argv=None):
    ...
    os.system("cp  $CMSSW_BASE/src/ZZMatrixElement/MELA/data/$SCRAM_ARCH/*.so $CMSSW_BASE/lib/$SCRAM_ARCH")
...
#SVFitStandAlone outputFile="WZ.root" newOutputFile=1.0 newFile="none"
    bashScript = "#!/bin/bash\n value=$(<$INPUT)\n echo \"$value\"\n"
    #bashScript += '$CMSSW_BASE/bin/$SCRAM_ARCH/SVFitStandAlonePUATauDM inputfile=$value newFile=\'$OUTPUT\'' #% (channel, sample_name, period)
    bashScript += '$CMSSW_BASE/bin/$SCRAM_ARCH/MELA_StandAlone inputFile=$value newOutputFile=1.0 newFile=\'$OUTPUT\'' #% (channel, sample_name, period)
    bashScript += '\n'
