...
class DataCardCreatorHThTh_2016 {
	public:
		...
		DataCardCreatorHThTh_2016(optutl::CommandLineParser parser) {
			...
			FFweight2_      = parser.stringValue("FFweight2");
...
tauPtCut='(((pt_2*0.982)>40&&decayMode_2==0)||((pt_2*1.01)>40&&decayMode_2==1)||((pt_2*1.004)>40&&decayMode_2==10))&&(((pt_1*0.982)>50&&decayMode_1==0)||((pt_1*1.01)>50&&decayMode_1==1)||((pt_1*1.004)>50&&decayMode_1==10))&&'
		...
		*/
		...
		bool runQCD(string preSelection, string prefix, string zShape, float topExtrap, BkgOutput &output, string categorySelection, string qcdSelection, string qcdSelectionData) 
		{
			...
			pair<float,float>YieldSSLoose = estimateQCDYield("SS_Loose_Region", fullRelaxedSelectionDataSS+"&&"+qcdSelectionMax_, fullRelaxedSelectionSS+"&&"+qcdSelectionMax_,topExtrap,leg1Corr,prefix);
			cout<<"     Data Events in SS Signal QCD loose sideband region = "<<YieldSSLoose.first<<" +/- " <<YieldSSLoose.second<<endl;
			pair<float,float>YieldOSLoose = estimateQCDYield("OS_Loose_Region", fullRelaxedSelectionDataOS+"&&"+qcdSelectionMax_, fullRelaxedSelectionOS+"&&"+qcdSelectionMax_,topExtrap,leg1Corr,prefix);
			cout<<"     Data Events in OS Signal QCD loose sideband region = "<<YieldOSLoose.first<<" +/- " <<YieldOSLoose.second<<endl;
			pair<float,float>YieldSSTight = estimateQCDYield("SS_Tight_Region", fullSelectionDataSS       +"&&"+qcdSelectionMax_, fullSelectionSS+"&&"+qcdSelectionMax_,topExtrap,leg1Corr,prefix);
		...
		pair<float,float> estimateQCDYield(string text, string dataSelection, string mcSelection, float topExtrap,float leg1Corr, string prefix){
		  float tauIDCorr=tauID_*tauID_;
		  cout<<endl;
		  ...
		  pair<float,float> dataQCD = createHistogramAndShifts(dataFile_,   "QCD"+text,"("+dataSelection+")",scaleUp_,prefix); 
		  ...
		  pair<float,float> ZQCD    = createHistogramAndShifts(zttFile_,   "ZQCD"+text,"("+mcSelection+")*"+weight_+"*"+Zweight_,  luminosity_*tauIDCorr*zttScale_,prefix); 
		  ...
		  pair<float,float> TopQCD  = createHistogramAndShifts(topFile_, "TopQCD"+text,"("+mcSelection+")*"+weight_+"*"+TTweight_, luminosity_*tauIDCorr*topExtrap,prefix); 
		  ...
		  pair<float,float> VVQCD   = createHistogramAndShifts(vvFile_,   "VVQCD"+text,"("+mcSelection+")*"+weight_,               luminosity_*tauIDCorr*vvScale_,prefix,false);
		  cout<<"          VV Yield    - "<<VVYield.first<<endl;

		  //W
		  pair<float,float> WYield    = getYield(wFile_,   "("+mcSelection+")*"+weight_,               luminosity_*tauID_);
		  pair<float,float> WQCD    = createHistogramAndShifts(wFile_,     "WQCD"+text,"("+mcSelection+")*"+weight_,               luminosity_*leg1Corr,prefix,false);
		...
		bool FFSyst(string channel, string prefix, string histo1, float corr, float unc)
		{
			...
			TH1F * h1 =(TH1F*) fout_->Get((channel+prefix+"/"+histo1).c_str());
			TH1F * h2 = (TH1F*)h1->Clone(TString(histo1+string("_CMS_htt_FFShape_"+chan+prefix+"_"+energy_+"Up")));
			TH1F * h3 = (TH1F*)h1->Clone(TString(histo1+string("_CMS_htt_FFShape_"+chan+prefix+"_"+energy_+"Down")));
			cout << "Channel label is " << chan << endl;
			float x,y;
			float cUp = corr+unc;
			float cDown = corr-unc;
	...
	private:
        ...
        string ffSelection_;   
        string ffSelectionMax_;
        string ffSelectionData_;
