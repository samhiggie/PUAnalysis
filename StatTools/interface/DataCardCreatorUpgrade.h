...
class DataCardCreatorHTT {
    public:
        ...
        DataCardCreatorHTT(optutl::CommandLineParser parser) {
            ...
            preselectionShape = parser.stringValue("preselectionShape");
	    ...
	    topScale_      = parser.doubleValue("topSF");
	...
	////// the only upgrade running you neeed!!
        void makeUpgradeShape(std::string preselection, 
			      std::string qcdShapeTightSel,
			      std::string qcdShapeLooseSel,
			      std::string qcdYieldLooseSel,
			      std::string prefix) {
	  BkgOutput output(0);
	  ...
	  std::cout<<"Make Higgs Shape"<<std::endl;
	  std::pair<float,float> ggHY   = createHistogramAndShifts(dir_+"ggH125.root",  "ggH125_Y",  ("("+preselection+")*"+weight_),luminosity_*3.12,prefix);
	  std::pair<float,float> ggH    = createHistogramAndShifts(dir_+"ggH125.root",  "ggH125",  ("("+preselectionShape+")*"+weight_),luminosity_*3.12,prefix);
	  renormalizeHistogram(filelabel_+prefix,"ggH125",ggHY.first);
	  ggH.first  = ggHY.first;
	  ggH.second = ggHY.second;

	  std::pair<float,float> vbfHY  = createHistogramAndShifts(dir_+"vbfH125.root", "qqH125_Y", ("("+preselection+")*"+weight_),luminosity_*0.268,prefix);
	  std::pair<float,float> vbfH   = createHistogramAndShifts(dir_+"vbfH125.root", "qqH125", ("("+preselectionShape+")*"+weight_),luminosity_*0.268,prefix);
	  renormalizeHistogram(filelabel_+prefix,"qqH125",vbfHY.first);
	  vbfH.first  = vbfHY.first;
	  vbfH.second = vbfHY.second;

	  std::pair<float,float> zjetsY = createHistogramAndShifts(dir_+"DYJets.root",   "ZTT_Y",   ("("+preselection+"&&gen_match_1==5&&gen_match_2==5)*"+weight_),luminosity_*1*zttScale_,prefix);
	  std::pair<float,float> zjets  = createHistogramAndShifts(dir_+"DYJets.root",   "ZTT",   ("("+preselectionShape+"&&gen_match_1==5&&gen_match_2==5)*"+weight_),luminosity_*1*zttScale_,prefix);
	  renormalizeHistogram(filelabel_+prefix,"ZTT",zjetsY.first);
	  zjets.first  = zjetsY.first;
	  zjets.second = zjetsY.second;

	  std::pair<float,float> zllY   = createHistogramAndShifts(dir_+"DYJets.root",   "ZL_Y",   ("("+preselection+"&&(gen_match_1!=5||gen_match_2!=5))*"+weight_),luminosity_*1*zttScale_,prefix);
	  std::pair<float,float> zll    = createHistogramAndShifts(dir_+"DYJets.root",   "ZL",   ("("+preselectionShape+"&&(gen_match_1!=5||gen_match_2!=5))*"+weight_),luminosity_*1*zttScale_,prefix);
	  renormalizeHistogram(filelabel_+prefix,"ZL",zllY.first);
	  zll.first  = zllY.first;
	  zll.second = zllY.second;

	  //std::pair<float,float> wjets = createHistogramAndShifts(dir_+"wjets.root",   "wjets",   ("("+preselection+")*"+weight_),luminosity_*1,prefix);
	  std::pair<float,float> ttbarY = createHistogramAndShifts(dir_+"ttBar.root",   "TT_Y",   ("("+preselection+")*"+weight_),luminosity_*1*topScale_,prefix);
	  std::pair<float,float> ttbar  = createHistogramAndShifts(dir_+"ttBar.root",   "TT",   ("("+preselectionShape+")*"+weight_),luminosity_*1*topScale_,prefix);
	  renormalizeHistogram(filelabel_+prefix,"TT",ttbarY.first);
	  ttbar.first  = ttbarY.first;
	  ttbar.second = ttbarY.second;


	  std::pair<float,float> ttj = createHistogramAndShifts(dir_+"ttBar.root",   "TTj",   ("("+preselection+"&&(gen_match_1==6&&gen_match_2==6))*"+weight_),luminosity_*1*topScale_,prefix);
	  std::pair<float,float> ttl = createHistogramAndShifts(dir_+"ttBar.root",   "TTl",   ("("+preselection+"&&((gen_match_1<5||gen_match_2<5)||(gen_match_1==6&&gen_match_2!=6)||(gen_match_1!=6&&gen_match_2==6)))*"+weight_),luminosity_*1*topScale_,prefix);
	  std::pair<float,float> ttt = createHistogramAndShifts(dir_+"ttBar.root",   "TTt",   ("("+preselection+"&&(gen_match_1==5&&gen_match_2==5))*"+weight_),luminosity_*1*topScale_,prefix);
	  std::pair<float,float> qcdShapeLoose   = createHistogramAndShifts(dir_+  "QCD.root",   "QCD",     ("("+qcdShapeLooseSel+")*"+weight_),luminosity_*1,prefix);
	  std::pair<float,float> qcdShapeTight   = createHistogramAndShifts(dir_+  "QCD.root",   "qcd_tight",     ("("+qcdShapeTightSel+")*"+weight_),luminosity_*1,prefix);
	  std::pair<float,float> qcdYieldLoose   = createHistogramAndShifts(dir_+  "QCD.root",   "qcd_yield_loose",     ("("+qcdYieldLooseSel+")*"+weight_),luminosity_*1,prefix);

	  std::cout<<std::endl;
	  /*
	  std::cout<<"::::::::Running QCD::::::::"<<std::endl;
	  std::cout<<"QCD Shape Tight = "<<qcdShapeTight.first<<" +/- "<<qcdShapeTight.second<<std::endl;
	  std::cout<<"QCD Shape Loose = "<<qcdShapeLoose.first<<" +/- "<<qcdShapeLoose.second<<std::endl;
	  std::cout<<"QCD Yield Loose = "<<qcdYieldLoose.first<<" +/- "<<qcdYieldLoose.second<<std::endl;
	  std::cout<<"Final QCD Yield = ("<<qcdShapeTight.first <<" / "<<qcdShapeLoose.first<<") x "<<qcdYieldLoose.first<<std::endl;
	  std::cout<<"                =  ("<<qcdShapeTight.first / qcdShapeLoose.first<<")  x  "<<qcdYieldLoose.first<<std::endl;
	  std::cout<<"-----------------------------------------"<<std::endl;
	  std::cout<<"                          "<<(qcdShapeTight.first/qcdShapeLoose.first)*qcdYieldLoose.first<<std::endl;
	  */
	  //float qcdSetYield = 0.00001;
	  /*
	  if((qcdShapeTight.first/qcdShapeLoose.first)*qcdYieldLoose.first > qcdSetYield)
	    qcdSetYield = (qcdShapeTight.first/qcdShapeLoose.first)*qcdYieldLoose.first;
	  */
	  //if(qcdShapeTight.first > 0.00001)
	  //qcdSetYield = qcdShapeTight.first; 
	    renormalizeHistogram(filelabel_+prefix,"QCD",qcdShapeTight.first*qcdFactor_);
	  //std::pair<float,float> qcd = std::make_pair((qcdShapeTight.first/qcdShapeLoose.first)*qcdYieldLoose.first,0);

	  std::pair<float,float> qcd = std::make_pair(qcdShapeTight.first,qcdShapeTight.second);
	  std::cout<<"Made Higgs Shape"<<std::endl;

	  float bkgrdErr  = sqrt(zjets.second*zjets.second+ttbar.second*ttbar.second+qcd.second*qcd.second);
	  //float bkgrdErr  = sqrt(zjets.second*zjets.second+wjets.second*wjets.second+ttbar.second*ttbar.second+qcd.second*qcd.second);
	  float signalErr = sqrt(ggH.second*ggH.second+vbfH.second*vbfH.second);
	  
	  std::cout<<"=============Output Yields============="<<std::endl;
	  std::cout<<"vbf   - "<<vbfH.first<<std::endl;
	  std::cout<<"ggH   - "<<ggH.first<<std::endl;
	  std::cout<<"zjets - "<<zjets.first<<std::endl;
	  std::cout<<"zll   - "<<zll.first<<std::endl;
	  //std::cout<<"wjets - "<<wjets.first<<std::endl;
	  std::cout<<"ttbar - "<<ttbar.first<<std::endl;
	  std::cout<<"qcd   - "<<qcdShapeTight.first*qcdFactor_<<std::endl;
	  std::cout<<std::endl;
	  std::cout<<"ttl - "<<ttl.first<<std::endl;
	  std::cout<<"ttj - "<<ttj.first<<std::endl;
	  std::cout<<"ttt - "<<ttt.first<<std::endl;
	  std::cout<<"======================================="<<std::endl;
	  //std::cout<<"background total = "<<zjets.first+wjets.first+ttbar.first+qcd.first<<" +/- "<<bkgrdErr<<std::endl;
	  std::cout<<"background total = "<<zjets.first+ttbar.first+qcd.first<<" +/- "<<bkgrdErr<<std::endl;
	  std::cout<<"signal total     = "<<vbfH.first+ggH.first<<" +/- "<<signalErr<<std::endl;
        }
