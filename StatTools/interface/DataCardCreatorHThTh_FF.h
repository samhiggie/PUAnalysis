...
class DataCardCreatorHThTh_2016 {
	public:
		...
		BkgOutput runOSLSMT(string preSelection,string prefix,string zShape, float topExtrap) {
            ...
            //Running the Fake Factor Method 
            //histos must be booked first ... 
            std::cout<<"Getting fractions for mvis and njets bins "<<std::endl;
            std::vector<double> fractions = getFractions(input cuts ... );
            computeFF();  
...
tauPtCut='(((pt_2*0.982)>40&&decayMode_2==0)||((pt_2*1.01)>40&&decayMode_2==1)||((pt_2*1.004)>40&&decayMode_2==10))&&(((pt_1*0.982)>50&&decayMode_1==0)||((pt_1*1.01)>50&&decayMode_1==1)||((pt_1*1.004)>50&&decayMode_1==10))&&'
		...
		*/
        ...
        //Function to calculate fractions for the fake factor object will do it for all bins!
        //  
        //returned vector will be <frac_w,frac_tt,frac_real,frac_qcd>
        //std::vector<double> getFFractions(string wjets,string zjets,string diboson,string ttbar,string data, string cut,float scaleFactor = 1, string postfix = "",bool normUC  = true, bool keys=false,bool ShapeUncertainty=true) {

        std::vector<float> getFFractions(string folder, string cut,float scaleFactor = 1, string postfix = "",bool normUC  = true, bool keys=false,bool ShapeUncertainty=true) {
            ...
            */
            ...
            TH1 *hw = (TH1F*) fout_->Get((folder+"/"+"W_FF"));
            TH1 *hzj = (TH1F*) fout_->Get((folder+"/"+"ZJ_FF"));
            TH1 *hvvj = (TH1F*) fout_->Get((folder+"/"+"VVJ_FF"));
            TH1 *hvvt = (TH1F*) fout_->Get((folder+"/"+"VVT_FF"));
            TH1 *httj = (TH1F*) fout_->Get((folder+"/"+"TTJ_FF"));
            TH1 *hztt = (TH1F*) fout_->Get((folder+"/"+"ZTT_FF"));
            TH1 *httt = (TH1F*) fout_->Get((folder+"/"+"TTT_FF"));
            TH1 *hdata = (TH1F*) fout_->Get((folder+"/"+"data_obs_FF"));

            std::vector<float> fractions;

            float m_vis[12] = [0,50,80,100,110,120,130,150,170,200,250,1000];
            float njets[4] = [-0.5,0.5,1.5,15];
            ...
            fractions.pushback(frac_w);
            fractions.pushback(frac_tt);
            fractions.pushback(frac_real);
            fractions.pushback(frac_qcd);

            
            return fractions; 
        }

        //function that applies the fakefactor object weight to events 
        void computeFF(string folder, std::vector<float> fractions,string cuts, ...){
            //creating fakefactor object
            FakeFactor ff;  
        
            //adding the variables necessary to calculate weight
            Float_t pt_1, pt_2, tau_pt, decayMode_1, decayMode_2, njets, mvis, mt, lep_iso;
            TBranch *b_pt_1, *b_pt_2, *b_tau_pt, *b_decayMode_1, *b_decayMode_2, *b_njets, *b_mvis, *b_mt, *b_lep_iso;
            float frac_w = (W + ZJ + VVJ) / data
            float frac_tt = TTJ / data
            float frac_real = (ZTT + TTT + VVT) / data
            float frac_qcd = 1. - (frac_w + frac_tt + frac_real)
            std::vector<float> fractions;
            std::vector<double>; input_vars(8);
            ...
            TH1 *ffhist = (TH1F*) fout_->Get((folder+"/"+"data_obs"));
            ffhist->SetName("fakefactor");
			fout_->cd(folder.c_str());
			ffhist->Write(h->GetName(),TObject::kOverwrite);
            

            
            
            //looping over data events 
            for(int i=0;i<t->GetEntries() ){
                t->GetEntry(i);
    
                //still pseudocode... 
                //need to apply cuts to match the regions ... 

                if( isAntiIsolated(tau1) and isIsolated(tau2) ){
                    inputs[0] = pt_1;
                    inputs[1] = pt_2; //pt of non-fake tau candidate
                    inputs[2] = decayMode_1;
                    inputs[3] = njets;
                    inputs[4] = mvis;
                    fractions  = getFFractions(m_vis, njets);
                    inputs[5] = fractions[3];//frac_qcd;
                    inputs[6] = fractions[0];//frac_w;
                    inputs[7] = fractions[1];//frac_tt;
                    ff_nom_1 = ff->value(inputs);

                }
                if( isIsolated(tau1) and isAntiIsolated(tau2) ){
                    inputs[0] = pt_2;
                    inputs[1] = pt_1; //pt of non-fake tau candidate
                    inputs[2] = decayMode_2;
                    inputs[3] = njets;
                    inputs[4] = mvis;
                    fractions  = getFFractions(m_vis, njets);
                    inputs[5] = fractions[3];//frac_qcd;
